...
//	console.log("s: ",s)
    ...
    var client1 = new kafka.KafkaClient({kafkaHost:kafka_config.broker_urls,requestTimeout:3000000});
...
/*function getResultConfirmation(bcContext,results){
	return new Promise(function(resolve,reject){
	...
	var client1 = new Client1('10.51.235.65:2181');
	var topics = [{
	topic: 'node-test',
	offset: offset_count
	}];
	...
	console.log("Topic offset:",topics[0].offset)
...
function invokebycontext(context, id, version, args, timeout){
	...
	.then((results) =>{
		...
		//console.log("size of result returned<client>: ",sizeof(results))
		pass_results = results;
		invoke_status.time_endorse = new Date().getTime()/1000;
		var proposalResponses = pass_results[0];
		//console.log("size of result returned<peer>: ",sizeof(proposalResponses[0]))
		...
		if (all_good) {
			...
			//console.log("size of result to orderer: ",sizeof(request))
...
function querybycontext(context, id, version, name,fname) {
	...
	//console.log("peer tagert is: ",targets[1])
